...
`$DOTFILES_DIR/README.md` を確認し、今回の同期内容に合わせて更新する。

### 更新が必要なケース

- 新しいスキルを追加した → 「収録スキル」テーブルに行を追加
- 新しいコマンドを追加した → 「収録コマンド」テーブルに行を追加
- 新しいフック・スクリプトを追加した → 「管理対象の設定」テーブルを更新
- README がまだ存在しない → 設定概要・機能概要・利用上の注意点・導入手順を含む README を新規作成

### 更新方針

- **差分だけ更新する** — 変更のないセクションは触らない
